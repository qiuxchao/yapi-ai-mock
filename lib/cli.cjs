...
function sortByWeights(list) {
  list.sort((a, b) => {
    ...
    const w = a.weights.reduce((w2, _, i) => {
      if (w2 === 0) {
        w2 = a.weights[i] - b.weights[i];
      }
      return w2;
    }, 0);
    return w;
...
class Generator {
              ...
              projectConfig.categories.map(async (categoryConfig, categoryIndex) => {
                ...
                const codes = (await Promise.all(
                  categoryIds.map(async (id, categoryIndex2) => {
                    ...
                    const groupedInterfaceCodes = vtils.groupBy(interfaceCodes, (item) => item.outputFilePath);
                ...
                for (const groupedCodes of vtils.values(vtils.groupBy(codes, (item) => item.outputFilePath))) {
  ...
  async generateInterfaceCode(syntheticalConfig, interfaceInfo) {
    ...
    const mockFunctionName = vtils.isFunction(syntheticalConfig.getMockFunctionName) ? await syntheticalConfig.getMockFunctionName(extendedInterfaceInfo, changeCase__namespace) : changeCase__namespace.camelCase(extendedInterfaceInfo.parsedPath.name);
