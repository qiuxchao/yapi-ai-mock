...
import { memoize, castArray, uniq, omit, isFunction, cloneDeepFast, groupBy, last, values, isEmpty, dedent, wait } from 'vtils';
...
class Generator {
  ...
  async generateInterfaceCode(syntheticalConfig, interfaceInfo) {
    ...
    const mockFunctionName = isFunction(syntheticalConfig.getMockFunctionName) ? await syntheticalConfig.getMockFunctionName(extendedInterfaceInfo, changeCase) : changeCase.camelCase(extendedInterfaceInfo.parsedPath.name);
